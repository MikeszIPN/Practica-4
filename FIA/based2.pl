...
%funciones
enfermedad(A,B,C,D,E,F,G,H,I,J,X):- bronquitis(A,B,C,D,X) ; tos(A,H,C,D,X) ; asma(A,I,H,X) ; bulimia(G,I,C,D,X) ; anorexia(G,F,C,D,X) ; cirrosis(E,B,G,X) ; gripa(C,D,H,J,X) ; dolorEstomacal(E,F,G,X) ; lesion(J,I,H,X).

diagnostico(A,B,C,D,E,F,G,H,I,J,Enfermedad,Tratamiento):- enfermedad(A,B,C,D,E,F,G,H,I,J,X) , nombre_enfermedad(X,Enfermedad) , tratamiento(Enfermedad,Tratamiento).